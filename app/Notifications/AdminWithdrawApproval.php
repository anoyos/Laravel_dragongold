...
class AdminWithdrawApproval extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                        ->subject('Approval request for ' . $this->user->name . " withdrawal")
...
//                      ->action('View Transaction', route('admin.transactions'));
    ...
    public function toArray($notifiable)
    {
        return [
            ...
            'type' => 'withdraw',
            'message' => "Please approve withdrawal of {$this->transaction->amount} {$this->currency} requested by {$this->user->name}"
        ];
    }
}
