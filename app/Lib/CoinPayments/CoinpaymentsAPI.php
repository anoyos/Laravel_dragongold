...
class CoinpaymentsAPI
{
    ...
    public function __construct($private_key, $public_key)
    {
        ...
        $this->request_handler = new CurlRequest($this->private_key, $this->public_key, $format);
    }
    ...
    public function GetUSDRates() {
        ...
        $response = $this->request_handler->execute('rates', $fields);
        if($response['error'] === 'ok'){
            $currs = ['BTC', 'ETH','LTC','TRX','XRP','EOS','ETC','BCH','BTG','ZEC','DASH'];
            $rawrates = $response['result'];
            $usd = $rawrates['USD']['rate_btc'];
            $rates = [];
            foreach($currs as $c){
                $rates[$c] = round($rawrates[$c]['rate_btc'] / $usd, 2);
            }
            return $rates;
        }
        return false;
    ...
    public function updateCurrencyRates($currencies) {
        $fields = [
            'short' => 1
        ];
        $response = $this->request_handler->execute('rates', $fields);
        if($response['error'] === 'ok'){
            ...
            foreach($currencies as $currency){
                $currency->usdrate = round($rawrates[$currency->symbol]['rate_btc'] / $usd, 2);
                $currency->save();
            }
            return true;
        }
