    ...
    $(".showBtn").click(function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (data, status) {
                            if (data) {
                                console.log(data);
                                $("#profileModal .name").text(data.fullname);
                                $("#profileModal .email").text(data.email);
                                $("#profileModal .status").text(data.status == 1 ? 'Active' : 'Blocked');
                                if(data.status == 1){
                                    $("#profileModal .status").addClass('btn-success');
                                }else{
                                    $("#profileModal .status").addClass('btn-danger')
                                }
                                $("#profileModal .modal-title").text(data.username.toUpperCase()+' Profile');
                                $("#profileModal .invested").text(data.totalInvested);
                                $("#profileModal .withdraw").text((data.totalWithdraw));
                                $("#profileModal .balance").text(data.balance);
            ...
            <ul class="twPc-Arrange">
                ...
                <li class="twPc-ArrangeSizeFit">
                    <a href="#" title="Total Withdraw">
                        ...
                        <span class="twPc-StatValue withdraw"></span>
                    </a>
                </li>
                <li class="twPc-ArrangeSizeFit">
                    <a href="#" title="Balance">
                        <span class="twPc-StatLabel twPc-block">Balance</span>
                        <span class="twPc-StatValue balance"></span>
                    </a>
                </li>
            </ul>
        </div>
