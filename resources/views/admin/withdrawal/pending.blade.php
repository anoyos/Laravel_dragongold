        ...
        <table class="table data-list-view">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="#" type="button" class="btn btn-icon btn-outline-info mr-1 mb-1"  data-toggle="tooltip" title="Approve" onclick="document.getElementById('approve').submit();"><i class="feather icon-check"></i></a>

                                        <a href="#" type="button" class="btn btn-icon btn-outline-danger mr-1 mb-1"  data-toggle="tooltip" title="Cancel" onclick="document.getElementById('decline').submit();"><i class="feather icon-x"></i></a>
                                         <a href="{{ route('admin.withdrawals.detail', $withdrawal->id) }}" class="btn btn-icon btn-outline-info mr-1 mb-1" data-toggle="tooltip" title="View"><i class="feather icon-eye"></i></a>

                        <form id="approve" method="POST" action="{{ route('admin.withdrawal.approve') }}">
                            @csrf
                            <input type="hidden" name="transaction" value="{{ $withdrawal->id }}">
                        </form>

                        <form id="decline" method="POST" action="{{ route('admin.withdrawal.cancel') }}">
                            @csrf
                            <input type="hidden" name="transaction" value="{{ $withdrawal->id }}">
                        </form>
                    </td>
